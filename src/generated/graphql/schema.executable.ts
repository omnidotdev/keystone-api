...
const siteIdentifier = sql.identifier("public", "site");
const spec_site = {
  name: "site",
...
const siteCodec = recordCodec(spec_site);
const userUniques = [{
  isPrimary: true,
  attributes: ["id"],
  description: undefined,
...
const resource_sitePgResource = registry.pgResources["site"];
...
const nodeIdHandler_Site = {
  typeName: "Site",
  codec: nodeIdCodecs_base64JSON_base64JSON,
  deprecationReason: undefined,
  plan($record) {
    return list([constant("Site", false), $record.get("id")]);
  },
  getSpec($list) {
    return {
      id: inhibitOnNull(access($list, [1]))
  ...
  get(spec) {
    return resource_sitePgResource.get(spec);
  },
  match(obj) {
    return obj[0] === "Site";
  }
};
const nodeFetcher_Site = $nodeId => {
  const $decoded = lambda($nodeId, specForHandler(nodeIdHandler_Site));
  return nodeIdHandler_Site.get(nodeIdHandler_Site.getSpec($decoded));
};
...
const colSpec5 = {
  fieldName: "rowId",
  attributeName: "id",
  attribute: spec_site.attributes.id
};
const colSpec6 = {
  fieldName: "organizationId",
  attributeName: "organization_id",
  attribute: spec_site.attributes.organization_id
};
const colSpec7 = {
  fieldName: "name",
  attributeName: "name",
  attribute: spec_site.attributes.name
};
...
const specFromArgs_Site = args => {
  const $nodeId = args.getRaw(["input", "id"]);
  return specFromNodeId(nodeIdHandler_Site, $nodeId);
};
...
export const objects = {
  Query: {
    ...
    plans: {
      ...
      site(_$root, {
        ...
        return resource_sitePgResource.get({
          id: $rowId
        });
      },
      siteById(_$parent, args) {
        const $nodeId = args.getRaw("id");
        return nodeFetcher_Site($nodeId);
      },
      sites: {
        plan() {
          return connection(resource_sitePgResource.find());
        },
        args: {
          first(_, $connection, arg) {
            $connection.setFirst(arg.getRaw());
  ...
  Mutation: {
    ...
    plans: {
      ...
      createSite: {
        plan(_, args) {
          const $insert = pgInsertSingle(resource_sitePgResource, Object.create(null));
          args.apply($insert);
          return object({
            result: $insert
          });
      ...
      deleteSite: {
        plan(_$root, args) {
          const $delete = pgDeleteSingle(resource_sitePgResource, {
            id: args.getRaw(['input', "rowId"])
          });
          args.apply($delete);
          return object({
      ...
      deleteSiteById: {
        plan(_$root, args) {
          const $delete = pgDeleteSingle(resource_sitePgResource, specFromArgs_Site2(args));
          args.apply($delete);
          return object({
            result: $delete
          });
      ...
      updateSite: {
        plan(_$root, args) {
          const $update = pgUpdateSingle(resource_sitePgResource, {
            id: args.getRaw(['input', "rowId"])
          });
          args.apply($update);
          return object({
      ...
      updateSiteById: {
        plan(_$root, args) {
          const $update = pgUpdateSingle(resource_sitePgResource, specFromArgs_Site(args));
          args.apply($update);
          return object({
            result: $update
          });
  ...
  CreateSitePayload: {
    ...
    plans: {
      ...
      siteEdge($mutation, fieldArgs) {
        return pgMutationPayloadEdge(resource_sitePgResource, siteUniques[0].attributes, $mutation, fieldArgs);
      }
    }
  },
  CreateUserPayload: {
  ...
  DeleteSitePayload: {
    ...
    plans: {
      ...
      deletedSiteId($object) {
        const $record = $object.getStepForKey("result"),
          specifier = nodeIdHandler_Site.plan($record);
        return lambda(specifier, nodeIdCodecs_base64JSON_base64JSON.encode);
      },
      query() {
        return rootValue();
  ...
  Site: {
    ...
    plans: {
      ...
      deployedUrl($record) {
        return $record.get("deployed_url");
      },
      designSystem($record) {
        return $record.get("design_system");
      },
      displayName($record) {
        return $record.get("display_name");
      },
      id($parent) {
        const specifier = nodeIdHandler_Site.plan($parent);
        return lambda(specifier, nodeIdCodecs[nodeIdHandler_Site.codec.name].encode);
      },
      organizationId($record) {
        return $record.get("organization_id");
      },
      publishState($record) {
        return $record.get("publish_state");
      },
      rowId($record) {
        return $record.get("id");
      },
    ...
    planType($specifier) {
      const spec = Object.create(null);
      for (const pkCol of siteUniques[0].attributes) spec[pkCol] = get2($specifier, pkCol);
      return resource_sitePgResource.get(spec);
    }
  },
  SiteConnection: {
  ...
  SiteInput: {
    ...
    plans: {
      ...
      deployedUrl(obj, val, {
        ...
        obj.set("deployed_url", bakedInputRuntime(schema, field.type, val));
      },
      designSystem(obj, val, {
        field,
        schema
      }) {
        obj.set("design_system", bakedInputRuntime(schema, field.type, val));
      },
      displayName(obj, val, {
        field,
        schema
      ...
      files(obj, val, {
        ...
        obj.set("files", bakedInputRuntime(schema, field.type, val));
      },
      name(obj, val, {
        field,
        schema
      ...
      publishState(obj, val, {
        field,
        schema
      }) {
        obj.set("publish_state", bakedInputRuntime(schema, field.type, val));
      },
      rowId(obj, val, {
        field,
        schema
      ...
      transcript(obj, val, {
        ...
        obj.set("transcript", bakedInputRuntime(schema, field.type, val));
      },
      updatedAt(obj, val, {
        field,
        schema
...
export const enums = {
  ...
  SiteOrderBy: {
    values: {
      ...
      PRIMARY_KEY_ASC(queryBuilder) {
        siteUniques[0].attributes.forEach(attributeName => {
          queryBuilder.orderBy({
            attribute: attributeName,
            direction: "ASC"
          });
